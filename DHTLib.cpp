 ...
 */
void DHTLib::begin(uint8_t pin, uint8_t type, uint8_t usec) {
  _pin = pin;
  _type = type;
#ifdef __AVR
 ...
 */
float DHTLib::readTemperature(bool S, bool force) {
  float f = NAN;
 ...
 */
float DHTLib::convertCtoF(float c) { return c * 1.8 + 32; }
 ...
 */
float DHTLib::convertFtoC(float f) { return (f - 32) * 0.55555; }
 ...
 */
float DHTLib::readHumidity(bool force) {
  float f = NAN;
  if (read(force)) {
    switch (_type) {
    case DHT11:
 ...
 */
float DHTLib::computeHeatIndex(bool isFahrenheit) {
 ...
 *	@return float heat index
 */
float DHTLib::computeHeatIndex(float temperature, float percentHumidity,
                            bool isFahrenheit) {
  float hi;
...
uint32_t DHTLib::expectPulse(bool level) {
#if (F_CPU > 16000000L)
  uint32_t count = 0;
#else
